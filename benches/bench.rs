...
use blake3_servil::OUT_LEN;
use blake3_servil::platform::{MAX_SIMD_DEGREE, Platform};
use blake3_servil::{BLOCK_LEN, CHUNK_LEN};
use rand::prelude::*;
use test::Bencher;
...
fn bench_many_chunks_fn(b: &mut Bencher, platform: Platform) {
    ...
    b.iter(|| {
        ...
        platform.hash_many(
            ...
            &[0; 8],
            0,
            blake3_servil::IncrementCounter::Yes,
            0,
            0,
            0,
            &mut out,
...
fn bench_many_parents_fn(b: &mut Bencher, platform: Platform) {
    ...
    b.iter(|| {
        ...
        platform.hash_many(
            ...
            &[0; 8],
            0,
            blake3_servil::IncrementCounter::No,
            0,
            0,
            0,
            &mut out,
...
fn bench_atonce(b: &mut Bencher, len: usize) {
    ...
    b.iter(|| blake3_servil::hash(input.get()));
}
...
fn bench_incremental(b: &mut Bencher, len: usize) {
    let mut input = RandomInput::new(b, len);
    b.iter(|| blake3_servil::Hasher::new().update(input.get()).finalize());
}
...
fn bench_rayon(b: &mut Bencher, len: usize) {
    let mut input = RandomInput::new(b, len);
    b.iter(|| blake3_servil::Hasher::new().update_rayon(input.get()).finalize());
}
...
fn bench_xof(b: &mut Bencher, len: usize) {
    ...
    let mut xof = blake3_servil::Hasher::new().finalize_xof();
