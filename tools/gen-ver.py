#!/usr/bin/env python3
# blake3-servil's copy of bench-hashes' tools/gen-ver.py (Zooko's release
# technique): a commit setting the version X.Y.Z, a second setting
# X.Y.Z+<first commit>, and a lightweight tag vX.Y.Z+<first commit> on the
# second. Changes for this repository: the crate is a library, so its own
# Cargo.lock is untracked and b3sum/Cargo.lock, which records the crate's
# version, is the lock file refreshed and committed, beside b3sum's
# requirement on the crate (b3sum/Cargo.toml, set to X.Y.Z; requirements
# take no build metadata); the first release (no earlier v tag) is allowed;
# and when the tree already carries X.Y.Z everywhere this script writes it,
# HEAD is the version commit. Upstream's tags (1.8.7) have no leading
# "v" and are ignored.
...
Update Cargo.toml and b3sum/Cargo.lock, commit the version changes, and
create a Git tag. NEW_VERSION must be greater than the current version and
...
def set_b3sum_requirement(manifest_path, new_version):
    """b3sum's dependency on this crate, `blake3 = { version = "...",
    path = "..", package = "blake3-servil", ... }`, to `new_version`."""
    b3sum_manifest = manifest_path.parent / "b3sum" / "Cargo.toml"
    text = b3sum_manifest.read_text(encoding="utf-8")
    lines = text.split("\n")
    matches = [
        index for index, line in enumerate(lines)
        if line.startswith("blake3 = {") and 'package = "blake3-servil"' in line
    ]
    if len(matches) != 1:
        fail(
            "b3sum/Cargo.toml must hold exactly one dependency on "
            f"blake3-servil; found {len(matches)}"
        )
    line = lines[matches[0]]
    start = line.index('version = "') + len('version = "')
    end = line.index('"', start)
    lines[matches[0]] = line[:start] + new_version + line[end:]
    b3sum_manifest.write_text("\n".join(lines), encoding="utf-8")
...
def refresh_lock_file_and_verify(
    ...
    # b3sum depends on this crate by path; resolving its graph rewrites the
    # version its Cargo.lock records.
    run(
        [
            "cargo",
            "metadata",
            ...
            "1",
            "--manifest-path",
            str(manifest_path.parent / "b3sum" / "Cargo.toml"),
        ],
        capture_output=True,
    )

    lock_path = manifest_path.parent / "b3sum" / "Cargo.lock"

    if not lock_path.is_file():
        fail("b3sum/Cargo.lock must exist after refreshing Cargo metadata")
...
def assert_only_release_files_changed():
    ...
    if not status:
        fail(
            ...
            "Cargo.toml or b3sum/Cargo.lock"
        )
    ...
    if unexpected_lines:
        fail(
            ...
            "Cargo.toml and b3sum/Cargo.lock:\n"
...
def commit_release_files(message):
    ...
    git(["add", "Cargo.toml", "b3sum/Cargo.toml", "b3sum/Cargo.lock"])
...
def main():
    ...
    if not (repo_root / "b3sum" / "Cargo.lock").is_file():
        fail("b3sum/Cargo.lock must exist before generating a release")
    ...
    if current_tag is not None and proposed_version <= current_version:
        fail(
    ...
    print(
        ...
        + (
            f"is greater than {format_numeric_version(current_version)}"
            if current_tag is not None
            else "is the first release"
        )
    )
    ...
    if git_status():
        commit_release_files(
            "Update Cargo.toml and b3sum/Cargo.lock "
            f"for version {base_version}"
        )
    else:
        print(f"The tree already carries {base_version}: HEAD is the version commit.")
